...
def export_data(series, pointers, nseg, show_pic, output_name):
    ...
    if show_pic == 1:
        ...
        for i in result.index:
            ...
            upper_band = result.loc[i, 'mean'] + np.full([size], result.loc[i, 'variance'])
            lower_band = result.loc[i, 'mean'] - np.full([size], result.loc[i, 'variance'])
...
def segment(data, show_steps = 0, show_pic = 0, output_name = "segmentation_result"):
    ...
    # pointers: countains the index numbers of the starting points of each segment.
